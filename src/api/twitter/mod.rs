...
use config;
use ::http;
use ::http::{AutoClient, Future, Mime, Request};
...
impl Twitter {
    ...
    pub fn upload_image(&self, _name: &str, _mime: &Mime, data: &[u8]) -> impl Future<Item = u64, Error = ()> {
        ...
        let req = Request::post(IMAGES_URL).expect("To create request")
                                           ...
                                           .form(&media).expect("To finalize request")
                                           .send();

        req.map_err(|error| eprintln!("Twitter: Upload error: {:?}", error))
    ...
    pub fn post(&self, message: &str, media_attachments: &[u64], flags: &PostFlags) -> impl Future<Item = (), Error = ()> {
        ...
        let req = Request::post(POST_URL).expect("To create request")
                                         .set_header(http::header::AUTHORIZATION, auth_header)
                                         .form(&tweet).expect("To create tweet data")
                                         .send();
